...
class Length(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr
    min: Optional[StrictInt] = Field(description="Shortest possible length, or null if empty.")
    max: Optional[StrictInt] = Field(description="Longest possible length, or null if unbounded.")
    __properties: ClassVar[List[str]] = ["type", "min", "max"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['length']):
            raise ValueError("must be one of enum values ('length')")
        return value
